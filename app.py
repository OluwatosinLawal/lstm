...
with st.sidebar:
    st.image("https://via.placeholder.com/200x60?text=Alerzo+LSTM",
...
if page == "🏠 Home":
    ...
    with col4:
        ...
        <div class="metric-card">
            ...
            <div class="metric-value">68.77%</div>
        </div>""", unsafe_allow_html=True)
    ...
    with c1:
        ...
        st.write("Upload your cleaned CSV sales data and the model will generate forecasts against the actual values, showing RMSE, MAE, and sMAPE on your data.")
    with c2:
        st.markdown("#### 🔮 Future Predictions")
        st.write("Upload your most recent sales data and generate forward-looking forecasts for the next 7, 14, or 30 days using the trained LSTM model.")
    with c3:
        st.markdown("#### 📊 Model Performance")
        st.write("View the final comparative results of the LSTM against ARIMA and Prophet baselines on the original test set.")
...
# =============================================================================
    ...
    st.markdown("Upload one or more cleaned CSV files to run the LSTM model and see how well it forecasts against actual sales.")

    if model is None:
        st.error("Model not loaded. Check that model/lstm_demand_forecast.keras and model/scaler.pkl exist.")
        st.stop()
    ...
    if uploaded_files:
        ...
        st.markdown(f"**Date range:** {daily['date'].min().date()} → {daily['date'].max().date()}")
        st.markdown(f"**Mean daily sales:** ₦{daily['total_sales'].mean():,.0f}")

        if len(daily) < look_back + 10:
            st.error(f"Not enough data. Need at least {look_back + 10} trading days, got {len(daily)}.")
            st.stop()
        ...
        m3.metric("MAPE", f"{mape:.1f}%", help="Filtered: excludes days below 1% of mean sales")
        m4.metric("sMAPE", f"{smape:.1f}%", help="Symmetric MAPE — robust to near-zero days")

        # ── Plot ────────────────────────────────────────────────
        st.markdown("### Forecast vs Actual Sales")
        fig, ax = plt.subplots(figsize=(14, 5))
        ax.plot(dates, y_actual, color="#1565C0", linewidth=1.0,
                label="Actual Sales", alpha=0.85)
        ax.plot(dates, y_pred,   color="#E53935", linewidth=1.2,
...
# =============================================================================
    ...
    st.markdown("Upload your most recent sales data to generate forward-looking predictions.")
    ...
    if uploaded:
        ...
        st.success(f"✔ {len(daily):,} trading days loaded | "
        ...
        st.markdown(f"Forecast period: **{future_dates[0].date()}** → **{future_dates[-1].date()}**")

        # ── Summary metrics ──────────────────────────────────────
        c1, c2, c3 = st.columns(3)
        c1.metric("Total forecast (period)",
                  f"₦{future_preds.sum():,.0f}")
        c2.metric("Average daily forecast",
                  f"₦{future_preds.mean():,.0f}")
        c3.metric("Peak day forecast",
                  f"₦{future_preds.max():,.0f}")
        ...
        # Forecast line
        ax.plot(future_df["date"], future_df["forecast_ngn"],
        ...
        ax.set_title(f"LSTM {n_days}-Day Demand Forecast — Alerzo",
        ...
        # ── Download ──────────────────────────────────────────────
        csv_bytes = future_df.assign(
            date=future_df["date"].dt.strftime("%d/%m/%Y")
...
# =============================================================================
    ...
    models  = ["Vanilla LSTM", "Prophet", "ARIMA"]

    metrics_data = [
        ("RMSE (₦)",  [66358732,  167442799, 207790264], "RMSE (₦)"),
        ("MAE (₦)",   [53692800,  144600650, 194490642], "MAE (₦)"),
        ("MAPE (%)",  [424.40,    1262.95,   1525.71],   "MAPE (%)"),
        ("sMAPE (%)", [68.77,     98.04,     109.89],    "sMAPE (%)"),
    ]

    for ax, (title, vals, ylabel) in zip(axes, metrics_data):
        bars = ax.bar(models, vals, color=colours, edgecolor="white", width=0.5)
        ax.set_title(title, fontsize=11, fontweight="bold")
        ax.set_ylabel(ylabel, fontsize=9)
        for bar in bars:
            h = bar.get_height()
            label = f"₦{h:,.0f}" if "₦" in ylabel else f"{h:.1f}%"
            ax.text(bar.get_x() + bar.get_width() / 2, h * 1.02,
                    label, ha="center", va="bottom", fontsize=7)
        ax.grid(True, alpha=0.3, axis="y")
        ax.tick_params(axis="x", labelsize=8)

    plt.suptitle("Model Comparison: LSTM vs Baseline Models",
